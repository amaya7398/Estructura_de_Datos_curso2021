...
public class TreeGenerico<T extends Comparable<T>> {
    ...
    public Node<T>[] findWithParent (T data){
        Node<T> current = root;
        Node<T> parent = null;
        @SuppressWarnings("unchecked")
        Node<T>[] CuPa = (Node<T>[]) Array.newInstance(Node.class, 2); //Node<T>[] Cu_Pa = (Node<T>[]) new Object<T>[2]; //No se puede, no hay tortillas
    ...
    public void delete(T data){ //PARENTandSON[0] = PARENT      PARENTandSON[1] = SON
        Node<T>[] parentAndSon = (Node<T>[]) Array.newInstance(Node.class, 2);  //[0]: PARENT [1]: SON
        ...
        if(parentAndSon[1] instanceof Node<?>){ //IF instaceOf NODE it was found
            if(twoChilds){  //DOS HIJOS
                Node<T> mostRight; //parentAndSon[1] == SON. The 
    ...
    private void deleteLeaftOrSon(Node<T>[] parentAndSon){
        boolean leaft = false;
    ...
    private Node<T> subTreeLeft(Node<T> toDelete){
        Node<T> parent; //If some child of subTREE have not right NODE
    ...
    private void displayNodes(Node<T> node){
...
*/
    System.out.println("============================================================");
        ...
        Node<String>[] both = a2.findWithParent("F");
